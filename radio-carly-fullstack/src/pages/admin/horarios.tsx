...
// Colores rotativos para distinguir clientes visualmente
const COLORES = [
...
export default function AdminHorarios() {
  ...
  // Asignar color fijo por cliente
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/admin/clientes" className="text-gray-400 hover:text-white transition-colors text-sm">← Clientes</Link>
            <h1 className="text-white font-bold text-2xl">Calendario semanal</h1>
          </div>
          <div className="flex gap-3 text-sm">
            <Link href="/admin/config" className="text-gray-400 hover:text-white transition-colors">Precios</Link>
            <button onClick={() => { sessionStorage.removeItem("admin"); router.push("/"); }} className="text-gray-400 hover:text-red-400 transition-colors">Salir</button>
          </div>
        </div>
                ...
                {clienteIds.map((id) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`text-xs font-semibold px-3 py-1 rounded-full border ${colorPorCliente[id]} hover:opacity-80 transition-opacity`}
                    >
                      {nombre}
                    </Link>
                  );
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-3">
              {DIAS.slice(1).map((dia, i) => {
                ...
                return (
                  <div
                    key={numDia}
                    className={`bg-gray-900 rounded-2xl p-3 space-y-2 ${esHoy ? "ring-2 ring-blue-500" : ""}`}
                  >
                    <div className="flex items-center justify-between">
                      <h2 className={`font-semibold text-sm ${esHoy ? "text-blue-400" : "text-gray-400"}`}>
                        {dia}
                      </h2>
                      {esHoy && (
                        <span className="text-xs bg-blue-600 text-white px-2 py-0.5 rounded-full">Hoy</span>
                      )}
                    </div>

                    {slots.length === 0 ? (
                      <p className="text-gray-700 text-xs">Sin sesiones</p>
                    ) : (
                      <div className="space-y-1.5">
                        {slots.map((h) => (
                          <Link
                            ...
                            className={`block border rounded-xl px-2.5 py-2 text-xs font-medium transition-opacity hover:opacity-75 ${colorPorCliente[h.idcliente]}`}
                          >
